    ...
    , ui(new Ui::onb2)
{
    ...
    ui->tw->setHorizontalHeaderItem(3, new QTableWidgetItem("Почта"));
    ui->tw->setHorizontalHeaderItem(4, new QTableWidgetItem("Часы работы"));
...
void onb2::selectAll()
{
    ...
    QString sqlstr = "SELECT * FROM service_centers ORDER BY servicecenter_id";
    ...
    while (query.next())
    {
        ...
        QTableWidgetItem *idItem = new QTableWidgetItem(query.value("servicecenter_id").toString());
        ui->tw->setItem(rowCount, 0, idItem);

        // Заполняем значениями из базы данных
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("location").toString()));
        ui->tw->setItem(rowCount, 2, new QTableWidgetItem(query.value("contact_number").toString()));
        ui->tw->setItem(rowCount, 3, new QTableWidgetItem(query.value("email").toString()));
        ui->tw->setItem(rowCount, 4, new QTableWidgetItem(query.value("operating_hours").toString()));
...
void onb2::add()
{
    ...
    QString sqlstr = "INSERT INTO service_centers(location, contact_number, email, operating_hours ) VALUES (?, ?, ?, ?)"; // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb2::remove()
{
    ...
    QString sqlstr = "DELETE FROM service_centers WHERE servicecenter_id = '" + pu1 + "'";
    // Выполняем запрос
    if (!query.exec(sqlstr))
    {
...
void onb2::edit()
{
    ...
    QString sqlstr = "UPDATE service_centers SET location = ?, contact_number = ?, email = ?, operating_hours =? WHERE servicecenter_id = ?";
...
void onb2::generateReportRequest() {
    ...
    QString reportText = "Адреса сервисных центров:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет об адрессах сервичных центров:\n";

    // Добавляем информацию
    for (int row = 0; row < ui->tw->rowCount(); ++row) {
        QString pu1 = ui->tw->item(row, 1)->text();
        QString pu2 = ui->tw->item(row, 2)->text();
        reportText +=  " адресс: " + pu1 + " Номер телефона" + pu2+";\n";
    }

    reportText += "\nС уважением,\n";
    reportText += "Управляющий отделом";

    QString fileName = "service_centers_report_request.txt"; // Место проведения файла для сохранения отчета
