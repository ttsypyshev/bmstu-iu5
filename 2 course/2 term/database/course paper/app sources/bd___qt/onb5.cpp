    ...
    , ui(new Ui::onb5)
{
    ...
    ui->tw->setHorizontalHeaderItem(2, new QTableWidgetItem("Производитель"));
    ui->tw->setHorizontalHeaderItem(3, new QTableWidgetItem("Модель"));
    ui->tw->setHorizontalHeaderItem(4, new QTableWidgetItem("Номер"));
...
void onb5::selectAll()
{
    ...
    QString sqlstr = "select * from devices ORDER BY device_id";
    ...
    while (query.next())
    {
        ...
        QTableWidgetItem *idItem = new QTableWidgetItem(query.value("device_id").toString());
        ui->tw->setItem(rowCount, 0, idItem);

        // Заполняем значениями из базы данных
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("description").toString()));
        ui->tw->setItem(rowCount, 2, new QTableWidgetItem(query.value("manufacturer").toString()));
        ui->tw->setItem(rowCount, 3, new QTableWidgetItem(query.value("model").toString()));
        ui->tw->setItem(rowCount, 4, new QTableWidgetItem(query.value("serial_number").toString()));
        ui->tw->setItem(rowCount, 5, new QTableWidgetItem(query.value("price").toString()));
...
void onb5::add()
{
    ...
    QString sqlstr = "insert into devices(description, manufacturer, model, serial_number, price) values(?, ?, ?, ?, ?)";  // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb5::remove()
{
    ...
    QString sqlstr = "DELETE FROM devices WHERE device_id = '" + pu1 + "'";
...
void onb5::edit()
{
    ...
    QString sqlstr = "UPDATE devices SET description = ?, manufacturer = ?, model = ?, serial_number = ?, price = ? WHERE device_id = ?";
...
void onb5::generateReportRequest() {
    ...
    QString reportText = "Запрос на формирование отчета - о Девайсах:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет о Девайсах:\n";
    ...
    for (int row = 0; row < ui->tw->rowCount(); ++row) {
        ...
        QString pu4 = ui->tw->item(row, 4)->text();

        reportText +=  " Название: " + pu1 + " Производитель: " + pu2 + " Модель: " + pu3 + " Серийный номер: " + pu4 + ";\n";
    }

    reportText += "\nС уважением,\n";
    reportText += "Управлющий отдела АБОБА";

    QString filedescription = "devices_request.txt"; // Имя файла для сохранения отчета

    QFile file(filedescription);
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QTextStream out(&file);
        out << reportText;
        file.close();
        QMessageBox::information(this, "Успех", "Запрос на формирование отчета успешно сохранен в файле " + filedescription);
    } else {
