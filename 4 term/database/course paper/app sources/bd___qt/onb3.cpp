    ...
    , ui(new Ui::onb3)
{
    ...
    ui->tw->setHorizontalHeaderItem(1, new QTableWidgetItem("Имя"));
    ui->tw->setHorizontalHeaderItem(2, new QTableWidgetItem("Фамилия"));
    ui->tw->setHorizontalHeaderItem(3, new QTableWidgetItem("Отчество"));
    ui->tw->setHorizontalHeaderItem(4, new QTableWidgetItem("Почта"));
    ui->tw->setHorizontalHeaderItem(5, new QTableWidgetItem("Телефон"));
...
void onb3::selectAll()
{
    ...
    QString sqlstr = "SELECT * FROM clients ORDER BY client_id";
    ...
    while (query.next())
    {
        ...
        ui->tw->setItem(rowCount, 1, new QTableWidgetItem(query.value("first_name").toString()));
        ui->tw->setItem(rowCount, 2, new QTableWidgetItem(query.value("middle_name").toString()));
        ui->tw->setItem(rowCount, 3, new QTableWidgetItem(query.value("last_name").toString()));
        ui->tw->setItem(rowCount, 4, new QTableWidgetItem(query.value("email").toString()));
        ui->tw->setItem(rowCount, 5, new QTableWidgetItem(query.value("phone_number").toString()));
...
void onb3::add()
{
    ...
    QString sqlstr = "INSERT INTO clients(first_name, middle_name, last_name, email, phone_number) VALUES (?, ?, ?, ?, ?)"; // Формируем строку SQL-запроса для вставки данных в таблицу
...
void onb3::remove()
{
    ...
    QString sqlstr = "DELETE FROM clients WHERE client_id = '" + pu1 + "'";
    // Выполняем запрос
    if (!query.exec(sqlstr))
    {
...
void onb3::edit()
{
    ...
    QString sqlstr = "UPDATE clients SET first_name = ?, middle_name = ?, last_name = ?, email = ?, phone_number = ? WHERE client_id = ?";
...
void onb3::generateReportRequest() {
    ...
    QString reportText = "Запрос на формирование отчета - Клиентах:\n\n";
    reportText += "Уважаемые коллеги,\n\n";
    reportText += "Подготовлен отчет о клиентах, нашей комапнии:\n";
    ...
    for (int row = 0; row < ui->tw->rowCount(); ++row) {
        ...
        reportText +=  " ИМЯ: " + pu1 + " ФАМИЛИЯ: " + pu1 + " ОТЧЕСТВО: " + pu1 + " ПОЧТА: " + pu1 + ";\n";
    }

    reportText += "\nС уважением,\n";
    reportText += "Управляющий отдела";

    QString fileName = "clients_report_request.txt"; // Дисциплина файла для сохранения отчета
