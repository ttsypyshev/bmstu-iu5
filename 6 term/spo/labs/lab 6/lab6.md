...
* **Передачи файлов между клиентом и сервером по сети** (обычно по TCP/IP);
* Загрузки файлов **на сервер** или **с сервера**;
* **Управления файлами на удалённой системе** (создание, удаление, переименование);
* FTP поддерживает **аутентификацию** (логин/пароль) и может работать в **активном** или **пассивном** режиме.

---

### **2. Перечислите этапы FTP-соединения.**

Этапы соединения по FTP:

1. **Установка управления соединением**:

   * Клиент подключается к серверу на порт **21 (по умолчанию)**.
   * Происходит **аутентификация пользователя** (логин и пароль).

2. **Установление канала передачи данных**:

   * Открывается **второе соединение** для передачи данных:

     * В **активном режиме** — сервер подключается к клиенту.
     * В **пассивном режиме** — клиент подключается к порту, указанному сервером.

3. **Передача файлов**:

   * Команды `RETR`, `STOR`, `LIST` и др. управляют передачей.

4. **Завершение сессии**:

   * Соединение закрывается по команде `QUIT`.

---

### **3. Какова особенность сервера vsftpd?**

**vsftpd (Very Secure FTP Daemon)** — это один из самых безопасных и быстрых FTP-серверов для Linux.

**Особенности:**

* **Высокий уровень безопасности** (отсюда "Very Secure");
* **Поддержка анонимного и локального входа**;
* Может работать в **chroot-режиме** (ограничивает доступ пользователя к его домашней папке);
* **Поддерживает SSL/TLS** для защищённого соединения (FTPS);
* Простота настройки и высокая производительность;
* Подходит как для **малых**, так и для **нагруженных серверов**.
